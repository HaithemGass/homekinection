...
#define L0_OFF writeMAXReg(rGPIO,(readMAXReg(rGPIO) & 0xFE));
#define L0_ON writeMAXReg(rGPIO,(readMAXReg(rGPIO) | 0x01));
#define L1_OFF writeMAXReg(rGPIO,(readMAXReg(rGPIO) & 0xFD));
#define L1_ON writeMAXReg(rGPIO,(readMAXReg(rGPIO) | 0x02));
#define L2_OFF writeMAXReg(rGPIO,(readMAXReg(rGPIO) & 0xFB));
#define L2_ON writeMAXReg(rGPIO,(readMAXReg(rGPIO) | 0x04));
#define L3_OFF writeMAXReg(rGPIO,(readMAXReg(rGPIO) & 0xF7));
#define L3_ON writeMAXReg(rGPIO,(readMAXReg(rGPIO) | 0x08));
#define L0_BLINK writeMAXReg(rGPIO,(readMAXReg(rGPIO) ^ 0x01));
#define L1_BLINK writeMAXReg(rGPIO,(readMAXReg(rGPIO) ^ 0x02));
#define L2_BLINK writeMAXReg(rGPIO,(readMAXReg(rGPIO) ^ 0x04));
#define L3_BLINK writeMAXReg(rGPIO,(readMAXReg(rGPIO) ^ 0x08));

#define SETBIT(reg,val) writeMAXReg(reg,(readMAXReg(reg)|val));
#define CLRBIT(reg,val) writeMAXReg(reg,(readMAXReg(reg)&~val));

// ************ BUG FIX ************
//#define STALL_EP0 writeMAXReg(9,0x23); // Set all three EP0 stall bits--data stage IN/OUT and status stage
// BUG FIX 2-9-06. The above statement hard-codes the register number to 9, ignoring the fact that
// the writeMAXReg function expects the register numbers to be pre-shifted 3 bits to put them into the 5 MSB's of
// the SPI command byte. Here is the correction:

#define STALL_EP0 writeMAXReg(rEPSTALLS,0x23);	// Set all three EP0 stall bits--data stage IN/OUT and status stage
